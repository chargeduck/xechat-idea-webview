...
public class OverCommandHandler extends AbstractCommandHandler {
    ...
    public void process(String[] args) {
        // Vue 迁移：AbstractPanelComponent 已移除，改为通过 ToolAction 管理工具生命周期
        String currentToolId = UICache.currentToolId;
        if (currentToolId != null) {
            UICache.currentToolId = null;
            UICache.toolPanelVisible = false;
        }

        Game game = GameAction.getGame();
        Tools tools = ToolAction.getTools();
        if (game == null && tools == null && currentToolId == null) {
